-- Simplified Fees Collection System
...
-- Fee Categories (Monthly, Yearly, One-time)
CREATE TABLE fee_categories (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  org_id INTEGER NOT NULL REFERENCES organizations(id),
  name VARCHAR(50) NOT NULL, -- 'Monthly', 'Yearly', 'One-time'
  description TEXT,
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMP DEFAULT NOW()
);

-- Indexes for fee_categories
CREATE INDEX idx_fee_categories_org ON fee_categories(org_id);
CREATE INDEX idx_fee_categories_active ON fee_categories(is_active);

-- Enhanced Fees table with categories
CREATE TABLE fees (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  org_id INTEGER NOT NULL REFERENCES organizations(id),
  ...
  UNIQUE(group_id, subject_id, category_id) -- One fee per group per subject per category
);
...
BEGIN
  ...
  -- Note: Students keep these fees even if they leave the group later
...
-- Function to generate due dates for recurring fees
CREATE OR REPLACE FUNCTION generate_recurring_due_dates(
  fee_category_name VARCHAR(50),
  base_due_date DATE DEFAULT NULL
)
RETURNS DATE AS $$
DECLARE
  result_date DATE;
BEGIN
  ...
  IF base_due_date IS NULL THEN
    base_due_date := CURRENT_DATE;
  END IF;
  
  CASE fee_category_name
    WHEN 'Monthly' THEN
      -- For monthly fees, set due date to same day of next month
      -- If day doesn't exist in next month, use last day of month
      result_date := base_due_date + INTERVAL '1 month';
      -- Handle edge cases like Jan 31 -> Feb 28/29
      IF EXTRACT(DAY FROM result_date) != EXTRACT(DAY FROM base_due_date) THEN
        result_date := (DATE_TRUNC('month', result_date) + INTERVAL '1 month' - INTERVAL '1 day')::DATE;
      END IF;
      
    WHEN 'Yearly' THEN
      -- For yearly fees, set due date to same date next year
      result_date := base_due_date + INTERVAL '1 year';
      
    ELSE
      -- For one-time fees, use the provided date
      result_date := base_due_date;
  END CASE;
...
INSERT INTO fee_categories (org_id, name, description) VALUES 
(100001, 'Monthly', 'Recurring monthly fees'),
(100001, 'Yearly', 'Annual fees'),
(100001, 'One-time', 'One-time payments');
